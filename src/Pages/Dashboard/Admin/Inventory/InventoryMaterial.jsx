...
const InventoryMaterial = () => {
 ...
 const { t, i18n } = useTranslation();
    ...
    // API Hooks
    const { data: listsData, loading: loadingLists, refetch } = useGet({
    ...
    const {
        ...
        url: `${apiUrl}/admin/inventory/material/create_inventory`,
    });
    ...
    const {
        ...
        url: `${apiUrl}/admin/inventory/material/current`,
    });
    ...
    const {
        ...
        url: `${apiUrl}/admin/inventory/material/history`,
    });
    ...
    const {
        ...
        url: editingInventoryId ? `${apiUrl}/admin/inventory/material/inability_list/${editingInventoryId}` : null,
    });
    ...
    const {
        ...
        url: selectedHistoryInventory ? `${apiUrl}/admin/inventory/material/inability_list/${selectedHistoryInventory.id}` : null,
    });
    ...
    const {
        ...
        url: `${apiUrl}/admin/inventory/material/wested`,
    });
    ...
    const availableProducts = useMemo(() => {
        ...
        return (listsData?.materials || [])
            .filter((p) => categoryIds.includes(p.category_id))
            .map((p) => ({ value: p.id, label: p.name }));
    }, [listsData?.materials, selectedCategories]);
    ...
    const createAvailableProducts = useMemo(() => {
        ...
        return (listsData?.materials || [])
            .filter((p) => categoryIds.includes(p.category_id))
            .map((p) => ({ value: p.id, label: p.name }));
    }, [listsData?.materials, createCategories]);
    ...
    const handleCreateInventory = async () => {
        ...
        if (createType.value === "partial") {
            ...
            payload.category_materials = categoryIds;
            payload.materials = productIds;
        }
    ...
    const exportHistoryDetailsPDF = () => {
        ...
        autoTable(doc, {
            head: [["#", "Material", "Category", "Quantity", "Actual Qty", "Shortage", "Cost"]],
        ...
        doc.save(`inventory_material_details_${selectedHistoryInventory.id}_${new Date().toISOString().split("T")[0]}.pdf`);
    };
    ...
    const exportHistoryDetailsExcel = () => {
        ...
        const ws = XLSX.utils.json_to_sheet(
            data.map((item, idx) => ({
                "#": idx + 1,
                "Material": item.material || item.product || "—",
        ...
        XLSX.writeFile(wb, `inventory_material_details_${selectedHistoryInventory.id}_${new Date().toISOString().split("T")[0]}.xlsx`);
    };

    return (
        <div className="w-full p-6 pb-20">
            <TitlePage text={t("Inventory Material")} />
